...
class TextEditor(tk.Tk):
	def __init__(self):
		...
		self.bind('<Control-Key-d>', self.download_audio)
		self.bind('<Control-Key-m>', self.code_edi)
	...
	def load_icons(self):
		...
		self.code_icon = PhotoImage(file='icons/code.png')
	...
	def draw_menu(self):
		...
		file_menu.add_separator()
		file_menu.add_command(label='Download Audio', accelerator='Ctrl+D', compound=tk.LEFT, image=self.download_icon, command=self.download_audio)
		file_menu.add_separator()
		file_menu.add_command(label='Code', accelerator='Ctrl+M', compound=tk.LEFT, image=self.code_icon, command=self.code_edi)
		file_menu.add_separator()
		file_menu.add_command(label='Exit', accelerator='Alt+F4', compound=tk.LEFT, image=self.exit_icon,
							  command=self.quit)
	...
	def code_edi(self, event=None):
		top = Toplevel(self)
		top.title("Code Editor")
		top.iconbitmap('icons/coastline.ico')
		top.geometry("640x340+150+80")
		...
		def set_file_path(path):
			global file_path
			file_path = path
			print("file_path from set_file_path_function: ",file_path)
		...
		def run():
			global file_path
			print("from run function: ",file_path)
			if file_path == '':
		...
		code_input = Text(top, font = "consolas 15")
		code_input.place(x=90, y=0, width = 340, height=360)
		code_output = Text(top,font="consolas 15",bg="#323846",fg="lightgreen")
		code_output.place(x=430,y=0,width=220,height=360)
		
		Save = PhotoImage(file="icons/save_code.png")
		Open = PhotoImage(file="icons/open_code.png")
		Run = PhotoImage(file="icons/run.png")

		
		Button(top,image=Open,bg="#323846",bd=0,command=open_file).place(x=20,y=135)
		Button(top,image=Save,bg="#323846",bd=0,command=save).place(x=20,y=260)
		Button(top,image=Run,bg="#323846",bd=0,command=run).place(x=20,y=30)
	...
	def save_file(self, event=None):
		...
		engine.say(self.textbox.get('1.0', 'end-1c'))
		engine.runAndWait()
	...
	def download_audio(self, event=None):
		...
		tts.save('audio.mp3')
		#messagebox.showinfo('Text Editor','Your Text file is now saved as AUDIO file')
...
def show_about():
	...
	label = tk.Label(win, text='A modern day text Editor', fg='red',
				font=('Calibri', 12))
	label.grid(row=0, column=0)
	text = tk.Text(win, width=30, height=4)
	text.insert(tk.END, 'Author : Letchu Pkt \n')
...
def get_help():
	url = 'https://github.com/letchupkt'
	webbrowser.open(url)
